...
export class OrdersController {
  ...
  async findAll(@Query('status') status?: OrderStatus, @Query('type') type?: OrderType) {
  ...
  async create(@Body() dto: CreateOrderDto, @CurrentUser() user: { userId: string; restaurantId: string }) {
  ...
  @Roles(UserRole.CASHIER, UserRole.ADMIN)
  @HttpCode(HttpStatus.CREATED)
  async addItem(@Param('id') orderId: string, @Body() body: { productId: string; quantity: number; notes?: string }) {
  ...
  async removeItem(@Param('id') orderId: string, @Param('itemId') itemId: string) {
